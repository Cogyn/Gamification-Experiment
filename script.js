...
class MathTutorAI {
    ...
    setupTabSwitching() {
        ...
        const switchToTab = (targetTab) => {
            ...
            // Initialisiere "Meine Aufgaben" Tab
            if (targetTab === 'my-tasks') {
                if (window.taskUploadManager) {
                    window.taskUploadManager.loadTasks();
                }
            }

            // Initialisiere "Achievements" Tab
...
`;
    ...
    async analyzeTextInput() {
        ...
        try {
            ...
            // Gamification Hook: Theory Analysis completed
            try {
                const rewards = onTaskCompleted('theory', {
                    topic: textInput,
                    complexity: 3 // Default medium complexity
                });
                if (rewards) {
                    console.log('[Gamification] Theory analysis rewards:', rewards);
                }
            } catch (error) {
                console.error('[Gamification] Error awarding rewards:', error);
            }
...
Beantworte diese Frage unter Berücksichtigung des vorherigen Kontexts. Beziehe dich auf die ursprüngliche Aufgabe und Erklärung, wenn relevant.`;
    ...
    async analyzeImageInput() {
        ...
        try {
            ...
            // Gamification Hook: Error Analysis completed
            try {
                // Try to parse response to get step information
                let stepData = { steps: [] };
                try {
                    ...
                    console.log('[Gamification] Could not parse response structure, using defaults');
                }

                const rewards = onTaskCompleted('error_analysis', {
                    steps: stepData.steps || [],
                    topic: descriptionInput
                });
                if (rewards) {
                    console.log('[Gamification] Error analysis rewards:', rewards);
                }
            } catch (error) {
                console.error('[Gamification] Error awarding rewards:', error);
            }
    ...
    /**
     * Initialisiert den Abitur-Modus (Toggle + Dropdowns)
     */
    /**
...
- Aufgabentyp: ${taskTypeName}`;
        ...
        try {
            ...
            // Gamification Hook: Task Generation completed
            try {
                ...
                const rewards = onTaskCompleted('task_generation', {
                    topic: topic,
                    subtopic: subtopic,
                    difficulty: difficultyMap[difficulty] || 4
                });
                if (rewards) {
                    console.log('[Gamification] Task generation rewards:', rewards);
                }
            } catch (error) {
                console.error('[Gamification] Error awarding rewards:', error);
            }
