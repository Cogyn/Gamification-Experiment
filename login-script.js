    ...
    async handleLogin() {
        ...
        try {
            ...
            if (result.success) {
                ...
                // Warte etwas länger, um sicherzustellen dass Token gespeichert ist
                // und verifiziere dass die Session wirklich aktiv ist
                await new Promise(resolve => setTimeout(resolve, 300));

                // Verifiziere Session vor Weiterleitung
                const verifyUser = await this.authService.getCurrentUser();
                if (verifyUser) {
                    console.log('[LoginManager] Session verified, redirecting...');
                    setTimeout(() => {
                        window.location.href = 'index.html';
                    }, 500);
                } else {
                    console.error('[LoginManager] Session verification failed');
                    this.showMessage('Login erfolgreich, aber Session konnte nicht verifiziert werden. Bitte versuche es erneut.', 'error');
                }
            }
        } catch (error) {
    ...
    async handleRegister() {
        ...
        try {
            ...
            if (result.success) {
                ...
                if (result.token) {
                    // Warte und verifiziere Session
                    await new Promise(resolve => setTimeout(resolve, 300));
                    const verifyUser = await this.authService.getCurrentUser();
                    if (verifyUser) {
                        console.log('[LoginManager] Registration session verified, redirecting...');
                        setTimeout(() => {
                            window.location.href = 'index.html';
                        }, 500);
                    } else {
                        console.error('[LoginManager] Registration session verification failed');
                        this.showMessage('Registrierung erfolgreich! Bitte melde dich jetzt an.', 'success');
                        setTimeout(() => {
                            this.switchForm('login');
                        }, 1500);
                    }
